"""
This file and the build_transfer_model.py and simple_cnn.py were all used to to create a transfer learning model with imagenet.
Although these models run, I was getting poor results in the time that I had. These were not used in the final
project.
"""

from simple_cnn import build_model
import os
...
class ClassificationNet(object):
    ...
    def model_predict(self, img_path,model):
        ...
        top_3_text = '\n'.join(['{0}: {1:.2f}%'.format(name, percent) for name, percent in zip(top_3_names,top_3_percent)])
        return top_3_text
...
def main():
    train_folder = 'data/train_test_split/train'
    validation_folder = 'data/train_test_split/val'
    ...
    simple_cnn = ClassificationNet('all_night', target_size, augmentation_strength=0.2,
